<template>
  <div class="relative min-h-[80vh] group/crt font-mono text-foreground flex flex-col">
    ...
    <!-- Main Content Layout -->
    <div class="grid grid-cols-1 lg:grid-cols-[250px_1fr] xl:grid-cols-[300px_1fr] gap-8 xl:gap-16 flex-grow items-start pb-24">
      ...
      <aside class="hidden lg:block relative z-10 w-full min-w-0">
        <CornerFrame class="sticky top-24 bg-background">
          <div class="p-6">
            <div class="text-accent font-black tracking-tight mb-4 flex items-center gap-2 border-b border-dashed border-foreground/30 pb-2 uppercase">
            ...
            <div class="text-xs font-mono space-y-1 opacity-80">
              ...
              >
                <span class="text-muted-foreground mr-2 group-hover:text-accent">{{ i === (projects?.length ?? 0) - 1 ? '└──' : '├──' }}</span>
                <span class="truncate block w-full relative">
                  <span class="group-hover:opacity-100 opacity-0 text-accent transition-all absolute -left-3">></span>
                  {{ project.title }}
                </span>
              </NuxtLink>
            </div>
            ...
            <div class="mt-8 pt-6 border-t border-dashed border-foreground/20 text-[10px] leading-tight text-muted-foreground">
      ...
      <main class="space-y-12 relative z-10">
        ...
        <div v-if="projects?.length" class="space-y-16 lg:space-y-24">
          <NuxtLink
            ...
            :enter="{ opacity: 1, x: 0, transition: { duration: 500, delay: index * 100, ease: 'easeOut' } }"
          ...
          >
            ...
            <div class="p-6 md:p-8 relative z-10 flex flex-col h-full">
              ...
              <div class="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                <div class="w-full">
                  <div class="flex items-center gap-3 mb-2 md:mb-3">
                    <span class="text-accent group-hover:text-background font-bold font-mono text-[10px] md:text-xs uppercase tracking-widest bg-accent/10 group-hover:bg-background/20 px-2 py-0.5 transition-colors">
                      {{ project.role || 'CORE_MODULE' }}
                    </span>
                    <span class="font-mono text-[10px] md:text-xs opacity-50 w-6 text-center border-b border-foreground/20 group-hover:border-background/20 transition-colors">
                      {{ String(index + 1).padStart(2, '0') }}
                    </span>
                  </div>
                  
                  <h2 class="text-3xl md:text-5xl lg:text-6xl font-black uppercase tracking-tighter leading-none group-hover:underline underline-offset-4 decoration-2 break-words text-foreground group-hover:text-background transition-colors">
                    {{ project.title }}
                  </h2>
                </div>
                ...
                <div v-if="project.date" class="font-mono text-xs md:text-sm border border-foreground/20 group-hover:border-background/20 px-3 py-1 bg-background/50 group-hover:bg-background/10 backdrop-blur-sm shrink-0 opacity-80 group-hover:opacity-100 transition-colors mt-2 md:mt-0">
                  V.{{ new Date(project.date).getFullYear() || 'LATEST' }}
                </div>
              </div>
              
              <p class="text-base md:text-xl md:leading-relaxed opacity-80 mb-8 max-w-3xl font-sans mt-2">
                {{ project.description }}
              </p>
              ...
              <div class="flex flex-col sm:flex-row sm:items-end justify-between mt-auto pt-6 border-t-2 border-dashed border-foreground/20 group-hover:border-background/30 transition-colors gap-6 relative">
                ...
                <div class="flex flex-wrap gap-2 text-[10px] md:text-xs font-mono items-center uppercase">
                ...
                <div class="text-xs md:text-sm font-bold uppercase flex items-center justify-center gap-1.5 transition-colors bg-accent/10 px-3 py-1.5 bg-transparent border-2 border-transparent group-hover:border-background/20 mt-2 hover:bg-transparent">
                  <span class="leading-none tracking-widest">DEPLOY</span>
                  <span class="font-black group-hover:text-accent transition-colors ml-1 group-hover:translate-x-1 duration-200">↗</span>
                </div>
              </div>
            </div>
          </NuxtLink>
