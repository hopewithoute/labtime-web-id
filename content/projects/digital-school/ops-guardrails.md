...
Scheduling is the clearest example. Solver runs execute through queues, and the result is stored behind a retrieval path instead of being tied to a single open request.

That is a simple architectural decision with real operational payoff. The UI can poll or reconnect. The job completes independently. The platform does not recompute expensive work just because the browser lifecycle ended.
...
Attendance and device-related flows should not force heavy business processing onto the request that receives raw input. External integrations should stay small and explicit. The edge endpoint validates authenticity, acknowledges quickly, and hands off processing to the appropriate background path.
...
### The result
Long-running work can outlive a page. Integrations stay contained. Noisy traffic does not get to reshape the main application path.
